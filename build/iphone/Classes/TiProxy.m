...
//Rare exceptions to indicate a bug in the _rtdnext code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_rtdnext = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_rtdnext]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.rtdnext.module"
